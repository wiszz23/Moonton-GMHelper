...
async function run() {
  ...
  const [mysqlRows] = await mysqlConn.execute(
    'SELECT id, owner FROM userdata WHERE owner = ?',
    [OWNER]
  );
  console.log('[MySQL] SELECT results:');
  ...
  const pgResult = await pgClient.query(
    'SELECT id, owner FROM userdata WHERE owner = $1',
    [OWNER]
  );
  console.log('[PostgreSQL] SELECT results:');
  console.table(pgResult.rows);

  // Step 3 already done (COMMANDS constant defined above)

  console.log('\n=== Step 4: Update both databases ===\n');

  // MySQL UPDATE
  const [mysqlUpdateResult] = await mysqlConn.execute(
    'UPDATE userdata SET commands = ? WHERE owner = ?',
    [COMMANDS, OWNER]
  );
  console.log('[MySQL] UPDATE affected rows:', mysqlUpdateResult.affectedRows);

  // PostgreSQL UPDATE
  const pgUpdateResult = await pgClient.query(
    'UPDATE userdata SET commands = $1 WHERE owner = $2',
    [COMMANDS, OWNER]
  );
  console.log('[PostgreSQL] UPDATE affected rows:', pgUpdateResult.rowCount);
