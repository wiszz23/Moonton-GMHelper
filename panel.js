document.addEventListener('DOMContentLoaded', async () => {
  ...
  let commands = {};
  let personalCommands = []; // 个人指令
  let currentTab = 'general'; // 'general' | 'personal'
  let editingIndex = -1; // -1 = 添加模式，>=0 = 编辑指定索引
  let history = []; // 执行历史

  // ====== 接收父窗口传来的用户信息，自动填充角色ID ======
  window.addEventListener('message', (event) => {
    const data = event.data;
    if (!data || data.type !== 'GM_USER_INFO') return;
  ...
  modalConfirmBtn.addEventListener('click', async () => {
    ...
    try {
      if (typeof chrome !== 'undefined' && chrome.storage) {
        await chrome.storage.local.set({ gm_personal_commands: personalCommands });
      }
    } catch (e) {
      console.error('[GM助手] 保存个人指令失败:', e);
    }
  ...
  async function loadCommands() {
    try {
      ...
      console.log('[GM助手] 加载了', Object.keys(commands).length, '条指令');
    } catch (err) {
    ...
    try {
      const result = typeof chrome !== 'undefined' && chrome.storage
        ? await chrome.storage.local.get('gm_personal_commands')
        : {};
      personalCommands = (result.gm_personal_commands || []).map(cmd => ({
        name: cmd.name,
        text: cmd.text,
